...
class LevelListEmbed(discord.Embed):
    def __init__(self, interaction: ContextAdapter):
        ...
        if len(levels) == 0:
            ...
            for row in levels:
                description += bot.translator.getLocalString(interaction, "editLevelReward", [("level", str(row[0])), ("roleID", str(row[1]))])

        title = bot.translator.getLocalString(interaction, "editlevelTitle", [("guild", interaction.getGuild().name)])
...
class LevelListView(ClearView):
    def __init__(self, interaction: ContextAdapter, timeout:int = 300):
        ...
        self.deleteLevel.label = bot.translator.getLocalString(interaction, "editlevelDeleteLabel", [])
        self.interaction = interaction

        self.add_item(RefreshButton(editInteraction = interaction, refreshClass = LevelListEmbed, interaction=interaction))
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if(not interaction.getAuthor().bot):
            if(interaction.getAuthor().guild_permissions.manage_guild):
                ...
                await cursor.execute("DELETE FROM gld_reward WHERE guild_id = ? AND rew_level = ?", (interaction.getGuild().id, self.values[0]))
                await interaction.getClient().connection.commit()
                await cursor.close()

                content = bot.translator.getLocalString(interaction, "editlevelDeleted", [("level", str(self.values[0]))])
