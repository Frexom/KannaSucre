...
async def pruneFunction(interaction: ContextAdapter, user: Union[discord.Member, discord.User]):
        ...
        if(interaction.getGuild() is not None):
            if interaction.getAuthor().guild_permissions.manage_messages:
                if not user.guild_permissions.manage_messages:
                    ...
                    content = bot.translator.getLocalString(interaction, "pruneUser", [("number", str(mess_count)), ("user", user.name), ("guild", interaction.getGuild().name)])
        ...
        if(interaction.getGuild() is not None):
            if interaction.getAuthor().guild_permissions.manage_messages:
                if amount <= 50:
                    ...
                    try:
                        mess_count = len(await interaction.getChannel().purge(limit = amount, reason = "Requested by " + interaction.getAuthor().name))
...
async def kickFunction(interaction:ContextAdapter, user: Union[discord.Member, discord.User], reason: str = None):
    if not interaction.getAuthor().bot :
        if(interaction.getGuild() is not None):
