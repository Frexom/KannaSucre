    ...
    if not interaction.getAuthor().bot :
        ...
        if time_since > 7200 or pity >= 1:
            ...
            #Pokemon already captured
                ...
                desc += "\n"
                desc += bot.translator.getLocalString(interaction, "pokeExtraRolls", [("number", pokemon.rarity[0]*0.25)])
    ...
    if not interaction.getAuthor().bot:
        ...
        #Callback definition, and buttons generation
            ...
            async def prevCallback(interaction):
                ...
                pokemon.prev_alt()
                await interaction.editMessage(content = pokemon.shiny_link if pokemon.shiny else pokemon.link, embed = pokemon.get_pokeinfo_embed())
                await interaction.defer()
            ...
            await interaction.sendMessage(content = pokemon.shiny_link if pokemon.shiny else pokemon.link, embed = pokemon.get_pokeinfo_embed(), view = buttonView)
