...
async def hugFunction(interaction: ContextAdapter, user: Union[discord.Member, discord.User]):
  if not interaction.getAuthor().bot :
      ...
      if interaction.getAuthor() == user:
          title = bot.translator.getLocalString(interaction, "hugLonely", [("user", interaction.getAuthor().display_name)])
          e = discord.Embed(title=title)
          link = "https://media1.tenor.com/images/1506349f38bf33760d45bde9b9b263a4/tenor.gif"
          e.set_image(url=link)
      else:
          title = bot.translator.getLocalString(interaction, "hugUser", [("user", user.display_name), ("otherUser", interaction.getAuthor().display_name)])
          e = discord.Embed(title=title)
