    ...
    if not interaction.getAuthor().bot:
        ...
        if interaction.getAuthor() == user or user.id == bot.user.id:
            title = bot.translator.getLocalString(
                interaction,
                "hugLonely",
                [("user", interaction.getAuthor().display_name)],
            )
            ...
            title = bot.translator.getLocalString(
                ...
                [
                    ...
                    ("otherUser", interaction.getAuthor().display_name),
                ],
            )
            e = discord.Embed(title=title)
...
async def standFunction(bot, interaction: ContextAdapter):
    if not interaction.getAuthor().bot:
        ...
        await cursor.execute(
            "SELECT stand_id FROM sta_user WHERE user_id = ?",
        ...
        if stand_id == None:
            ...
            await cursor.execute(
                "INSERT INTO sta_user(user_id, stand_id) VALUES(?, ?)",
                (interaction.getAuthor().id, stand_id),
            )
            await bot.connection.commit()
        else:
            stand_id = stand_id[0]
        await cursor.execute(
            "SELECT stand_name, stand_link FROM sta_stand WHERE stand_id = ?",
            (stand_id,),
        )
        ...
        title = bot.translator.getLocalString(
            interaction,
            "userStand",
            [("user", interaction.getAuthor().display_name), ("standName", stand[0])],
        )
