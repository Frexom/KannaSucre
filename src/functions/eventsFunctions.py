...
async def setup_func(guild):
    ...
    await cursor.execute(
        "SELECT guild_id FROM dis_guild WHERE guild_id = ?", (guild.id,)
    )
        ...
        if interaction.getGuild() is not None:
            ...
            if enabled[0] == 1:
                ...
                if guildXP > 500 * guildLevel:
                    ...
                    roleName = ""
                    success = True
                    levelRole: discord.Role = interaction.getGuild().get_role(
                        guildLeveling[2]
                    )
                    if levelRole is not None:
                        try:
                    ...
                    if roleName == "" or not success:
                        ...
                        if not success:
                            ...
                            try:
                                await interaction.getGuild().owner.send(
                                    content=roleError
                                )
                            except Exception as e:
                                pass
