    ...
    if not message.author.bot:
        ...
        if (message.guild is not None):
            ...
            if enabled[0] == 1:
                ...
                FROM gld_level
                WHERE user_id = ? AND guild_id = ?""", (message.author.id, message.guild.id, ))
                ...
                await cursor.execute("UPDATE gld_level SET lev_xp = ?, lev_level = ? WHERE user_id = ? AND guild_id = ?", (guildXP, guildLevel, message.author.id, message.guild.id))
    ...
    if (not interaction.getAuthor().bot):
        ...
        if (interaction.getGuild() is not None):
            ...
            if enabled[0] == 1:
                ...
                FROM gld_level
                WHERE user_id = ? AND guild_id = ?""", (interaction.getAuthor().id, interaction.getGuild().id, ))
                guildLeveling = await cursor.fetchone()
                guildXP = guildLeveling[0]
                guildLevel = guildLeveling[1]
                ...
                await cursor.execute("UPDATE gld_level SET lev_xp = ?, lev_level = ? WHERE user_id = ? AND guild_id = ?", (guildXP, guildLevel, interaction.getAuthor().id, interaction.getGuild().id))
